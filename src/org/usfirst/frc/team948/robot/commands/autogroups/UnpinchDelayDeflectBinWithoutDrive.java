...
public class UnpinchDelayDeflectBinWithoutDrive extends CommandGroup implements RoboConstants{
	public UnpinchDelayDeflectBinWithoutDrive(double deflectSpeed, double deflectTime, boolean bin, DelayAndDrive driveCommand){
		if(bin){
			addSequential(new Pinch(Pincher.Position.Open));
			addSequential(new DelayForBinClear(HEIGHT_TO_CLEAR_ACQUIRER));
			addSequential(new DeflectBin(Direction.Right, deflectSpeed, driveCommand, 3.5));
		}
		addSequential(new Pinch(Pincher.Position.Open));
	}
	public UnpinchDelayDeflectBinWithoutDrive(double deflectSpeed, double deflectTime, boolean bin, DriveStraightDistance driveCommand){
		if(bin){
			addSequential(new Pinch(Pincher.Position.Open));
			addSequential(new DelayForBinClear(HEIGHT_TO_CLEAR_ACQUIRER));
	//		addSequential(new DeflectBin(Direction.Right, deflectSpeed, driveCommand, 3.0));
		}
		addSequential(new Pinch(Pincher.Position.Open));
	}
}
